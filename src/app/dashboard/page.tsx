...
export default async function Dashboard() {
  ...
  async function createCommitAndPR(formData: FormData) {
    ...
    // Load repos to find default branch
    let repos: Repo[] = [];
    {
    ...
    if (!createRefRes.ok && createRefRes.status !== 422) return;

    // 3) Commit file
    const contentB64 = Buffer.from(content, "utf8").toString("base64");
    const putFileRes = await gh(`https://api.github.com/repos/${owner}/${repo}/contents/${encodeURIComponent(filePath)}`, {
      method: "PUT",
      body: JSON.stringify({ message: commitMessage, content: contentB64, branch: newBranch }),
    });
    ...
    const prRes = await gh(`https://api.github.com/repos/${owner}/${repo}/pulls`, {
      method: "POST",
      body: JSON.stringify({ title: prTitle, head: newBranch, base: baseBranch, body: prBody }),
    });
    if (!prRes.ok) return;
  }
  ...
  return (
    <main style={{ maxWidth: 960, margin: "0 auto", padding: 24, fontFamily: "ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial" }}>
      <h1>Dashboard</h1>
      {!session ? (
        <div style={{ marginTop: 12 }}>
          <p>Anda belum login.</p>
          <form action="/api/auth/signin/github" method="post">
            <button type="submit" style={{ padding: "8px 14px" }}>Login dengan GitHub</button>
          </form>
        </div>
      ) : (
      ...
      <p>Langkah selanjutnya: tambah template makalah otomatis dan opsi update file.</p>
    </main>
  );
}
